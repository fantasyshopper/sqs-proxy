...
function messageToBuffer(l){
    var s = l.split("|"), q = s.shift(), m = s.join("|");
    if(m){
        BUFFERS[q] = BUFFERS[q] || [];
        BUFFERS[q].push(m);
        outputLog('BUFFER:', q, m);
    }
}
...
function dispatchQueue(q){
    ...
    while( BUFFERS[q].length > 0 ){
        m = BUFFERS[q].shift();
        sqs.request('SendMessage', {'MessageBody': m}, function(err,res){
            if(err){
                errorLog("ERROR:", err);
            }else{
                if(res.Error){
                    ...
                    outputLog("QUEUED:", q, m, "SQS-ID:", res.SendMessageResult.MessageId);
                }
            }
        });
    }
